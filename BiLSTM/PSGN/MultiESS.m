...
% ncan ��ʾ�ܵ����ʸ���
function  [X,fval] = MultiESS(fitnessfcn,nvars,nobj,narc,ncan,bh_option,Point)

% [B1,I1] = sort(Point,'descend');
% [B2,I2] = sort(Point1,'descend');
I2 = zeros(nobj,ncan);

for i = 1:nobj
    
    [B1,I1] = sort(Point(i,:),'descend');
    I2(i,:) = I1;
...
%     end
    ...
    solution_vector(i,:) = fitnessfcn(star(i,:),Point);
...
for i = 1: nobj
    ...
    solution_vector(i,:) = fitnessfcn(star(i,:),Point);
%     star(2,:) = I2(1:nvars);
%     solution_vector(2,:) = fitnessfcn(star(2,:),Point,Point1);
end
...
for i = 1:bh_option.maxgen
    ...
    for j = 1:bh_option.sizestar
        ...
        if rand < ELS
            ...
            k1 = 0.078*rand;
            k = randperm(nvars,ceil(k1*nvars));
            ...
            mid1 = gbest(:,M+1:end);
            mid1(k) = [];
            k3 = setdiff([1:ncan],mid1);

            k4 = randperm(length(k3),length(k));

            gbest(:,M+1:end) = [mid1,k3(k4)];

            gbest(:,1:M) = fitnessfcn(gbest(:,M+1:end),Point);

            
        end
        
        mid1 = intersect(gbest(:,M+1:end),star(j,:));
%         mid2 = union(gbest,star(j,:));
        mid2 = setdiff(gbest(:,M+1:end),mid1);
        mid3 = setdiff(star(j,:),mid1);
        if ~isempty(mid2)
            k = randperm(length(mid2),randperm(length(mid2),1))
            mid3(k) = mid2(k);
        end
...
%         end
        ...
        % calculate the fitness
        solution_vector(j,:) = fitnessfcn(star(j,:),Point);
    ...
    for j = 1:bh_option.sizestar
        ...
        if v == 1;
            ...
            if any(sum(abs(repmat(solution_vector(j,:),size(gbest_all,1),1) - gbest_all(:,1:M)) > R,2) ==0) %进入吸收边界:吸收并重新生�?
%                 if N == 1
